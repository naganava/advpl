...
@Description Tela para gerar ordem de serviço de desbobinamento
...
Private cOper 	:= Space(10)
	...
	oCbx1	:= TComboBox():New(101,115,bSetGet(cMaq),aVar,072,010, oDlg,,,,,,.T.,,,,,,,,,,'Máquina',1)
...
Local lRet := .T.
	...
	if pesoTot <> nQtde1 .or. qtdTot <> nQtde2
		alert('A quantidade/peso total não bate com as chapas')
		lRet := .F.
	endif
Return lRet
...
Local lMsHelpAuto 	:= .F.
	...
	for i := 1 to len(oGetDados:aCols)
		...
		If lMsErroAuto
			ALERT ('Apontamento não processado.')
			Mostraerro()
		endif
	next i
return
